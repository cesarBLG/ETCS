...
struct STMSpecificDataField
{
    NID_DATA_t NID_DATA;
    L_CAPTION_t L_CAPTION;
    std::vector<X_CAPTION_t> X_CAPTION;
    STMDataFieldValue value;
    N_ITER_t N_ITER;
    std::vector<STMDataFieldValue> values;
    void copy(bit_manipulator &w)
    {
        NID_DATA.copy(w);
        L_CAPTION.copy(w);
...
struct STMSpecificDataEntryRequest : ETCS_packet
{
    Q_FOLLOWING_t Q_FOLLOWING;
    N_ITER_t N_ITER;
    std::vector<STMSpecificDataField> fields;
