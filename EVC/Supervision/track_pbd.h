...
class PBD_target : public target
{
    ...
    PBD_target(distance start, distance end, double d_PBD, bool emergency, double grad) : start(start), end(end), d_PBD(d_PBD), restriction(0, start, end, false), target(/*start+d_PBD*/confidenced_distance(), 0, target_class::PBD)
    {
