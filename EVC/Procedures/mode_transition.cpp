...
void initialize_mode_transitions()
{
    ...
    c[12] = [](){return level == Level::N1 && MA && MA->EoA_ma && MA->EoA_ma->min<(d_minsafefront(*MA->EoA_ma)-L_antenna_front);};
    c[14] = [](){return !cab_active[0] && !cab_active[1] && V_est == 0 && sl_signal;};
    c[15] = [](){return mode_to_ack==Mode::OS && mode_acknowledged;};
    c[16] = [](){return (level == Level::N2 || level==Level::N3) && MA && MA->EoA_ma && MA->EoA_ma->min<d_minsafefront(*MA->EoA_ma);};
