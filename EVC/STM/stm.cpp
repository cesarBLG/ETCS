...
stm_object::stm_object()
{
    ...
    conditions["C16"] = [this] {return last_order && last_order->state != stm_state::DA && (last_order->state != stm_state::CS || !last_order->conditional) && get_milliseconds() - last_order_time > 10000;};
    conditions["D16"] = [this] {return last_order && last_order->state == stm_state::DA && get_milliseconds() - last_order_time > 5000;};
    conditions["E16"] = [this] {return last_order && last_order->state == stm_state::CS && last_order->conditional && !national_trip && get_milliseconds() - last_order_time > 5000;};
}
void fill_stm_transitions()
{
    ...
    stm_transitions.push_back({stm_state::CO, stm_state::FA, {"A16", "B16", "C16", "H16", "I16", "L16", "P16", "A17"}});
    stm_transitions.push_back({stm_state::DE, stm_state::FA, {"A16", "B16", "C16", "H16", "I16", "L16", "P16", "A17"}});
    ...
    stm_transitions.push_back({stm_state::DA, stm_state::CS, {"B4a", "B4b", "I4a", "A4b", "E4a", "K4a", "L4a"}});
        ...
        for (stm_transition &t : available) {
            ...
            if (type != "") {
                if (t.to != stm_state::NP && t.to != stm_state::PO && type != "A17") {
                    if (t.to == stm_state::CS) {
                        bool conditional = type == "A4b" || type == "B4b";
                        stm->last_order = {stm_state::CS, conditional};
                    } else {
                        stm->last_order = {t.to, false};
                    }
...
void stm_object::report_received(stm_state newstate)
{
    ...
    state = newstate;
    if (last_order && last_order->state == state)
        last_order = {};
...
void update_stm_control()
{
    ...
    for (auto kvp : installed_stms) {
        ...
        if (stm2->last_order && stm2->last_order->conditional && stm2->last_order->state == stm_state::CS && stm2->state != stm_state::CS && (stm2->state != stm_state::FA || V_est > 0))
            stm_control_EB = true;
